...
impl<DB: Database> SharedPool<DB> {
    pub(super) fn new_arc(
        ...
        // `self.close()` releases `usize::MAX / 2` permits to wake all waiters
        // if we had more connections than that, the permit amount will overflow
        // this should be optimized out on 64-bit platforms as `u32::MAX < (usize::MAX / 2)`
        assert!(capacity < (usize::MAX / 2), "are you insane?!?!");
    ...
    pub(super) async fn close(&self) {
        let already_closed = self.is_closed.swap(true, Ordering::Acquire);
        ...
        if !already_closed {
            ...
            self.semaphore.release(usize::MAX / 2);
        }

        // ensure we wait until the pool is actually closed
        while self.size() > 0 {
            if let Some(floating) = self.try_acquire() {
    ...
    fn pop_idle<'a>(&'a self, permit: SemaphoreReleaser<'a>) -> Result<Floating<'a, Idle<DB>>, SemaphoreReleaser<'a>> {
    ...
    pub(super) fn try_increment_size<'a>(&'a self, permit: SemaphoreReleaser<'a>) -> Result<DecrementSizeGuard<'a>, SemaphoreReleaser<'a>> {
        ...
        while size < self.options.max_connections {
            ...
            {
                Ok(_) => return Ok(DecrementSizeGuard::from_permit(self, permit)),
...
impl<'a> DecrementSizeGuard<'a> {
    ...
    pub fn from_permit<DB: Database>(pool: &'a SharedPool<DB>, mut permit: SemaphoreReleaser<'a>) -> Self {
